  ...
  try {
    ...
    switch (type) {
      case 'top_gainers': // 涨幅榜 - 取change_percent前5%（约25名）
        query = `
          SELECT ticker as symbol, name_zh as name, last_price as current_price, change_percent, market_cap
          FROM stocks 
      ...
      case 'top_turnover': // 成交额榜 - 取turnover前25名
        query = `
          SELECT ticker as symbol, name_zh as name, last_price as current_price, change_percent, market_cap, volume, turnover
          FROM stocks 
      ...
      case 'new_highs': // 创年内新高前15名
        query = `
          SELECT ticker as symbol, name_zh as name, last_price as current_price, change_percent, market_cap, week_52_high
          FROM stocks 
      ...
      case 'new_lows': // 创年内新低前15名
        query = `
          SELECT ticker as symbol, name_zh as name, last_price as current_price, change_percent, market_cap, week_52_low
          FROM stocks 
