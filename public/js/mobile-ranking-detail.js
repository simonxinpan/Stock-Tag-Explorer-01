// ================================================================
// == 移动版股票榜单详情页 - 基于桌面版trending.js的完美逻辑 ==
// ================================================================
...
function generateNavigationButtons(currentListType, currentMarket) {
  ...
  const navigationHTML = TRENDING_LISTS_CONFIG.map(config => {
    ...
    return `
      <a href="mobile-ranking-detail.html?market=${currentMarket}&type=${config.type}" 
...
async function main() {
    try {
        ...
        console.log("✅ 移动版榜单详情页脚本执行完成");

    } catch (error) {
        console.error("❌ 移动版详情页脚本主流程发生致命错误:", error);
    }
}
